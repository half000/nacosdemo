...
/**
 * 登录过滤器  权限类似  就是在处理过程中标记sendZuulResponse
 */
@Component
public class LoginFilter extends ZuulFilter {
    ...
    public Object run() throws ZuulException {
        ...
        if (StringUtils.isBlank(token)) {
            // 过滤该请求，不对其进行路由  这里如果是zuulservletfilter直接在prefilter后就返回，如果是zuulservlet会把过滤器都执行一遍，
            // 负责转发的2个RibbonRoutingFilter与SimpleHostRoutingFilter会跳过处理
